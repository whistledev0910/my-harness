...
verify="$root/scripts/verify-harness-cli-release-identity.sh"

[[ $# == 3 ]] || {
  echo "usage: $0 <harness-cli-vX.Y.Z> <source-sha> <proof-run-id>" >&2
  exit 2
}
...
if [[ -z "$remote_oid" ]]; then
  git tag -a "$tag" "$source_sha" \
    -m "Harness CLI $tag" \
    -m "proof-run=$proof_run source=$source_sha"
...
echo "Harness CLI release tag promoted: tag=$tag source=$source_sha proof-run=$proof_run"
