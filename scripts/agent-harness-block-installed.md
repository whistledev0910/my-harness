...
record. Read `.harness-core/docs/WORKFLOW.md` and only the product, design,
plan, code, and validation material relevant to the task.
...
- Create or update one file under `.harness-core/docs/plans/active/` when work
  spans sessions, needs coordination or an ordered sequence, has meaningful
  dependencies, or requires explicit recovery steps. Move it to
  `.harness-core/docs/plans/completed/` only after validation.
